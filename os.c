...
void *readfile(void *threadid) { 
  long tid;
  tid = (long)threadid;
  int a,b,c = 0; // to store 3 int from file
  while(keepreading == true)  //keep checking someone reached end of the file
  {
    ...
    if(Isreading) // if a thread is currently reading jump to beggging
    {
      continue; //jump to beggning
    }
...
int main ()
{
  ...
  int i;
  for( i = 0; i < NUM_THREADS; i++ ) { //thread creation
